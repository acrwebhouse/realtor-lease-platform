        ...
        if(response.data && response.data.data){
            ...
            for(let i = 0 ;i<items.length; i++){
                ...
                const item = {
                    key: i,
                    id: items[i]._id,
                    clientContent: [`租客:${items[i].clientName}`, `租客電話:${items[i].clientPhone}`],
        ...
        <div>
            <Row>
                ...
                <Col xs={24} sm={6} md={6} lg={5} xl={4}>
                    {showStartOrCount !== 0 ?
                        showStartOrCount === 1 ?
                        <Input id="startNo" onChange={(e) => getHousesArg.start=e.target.value} placeholder="第幾筆開始顯示（填數字）"  style={{
                        width: '100%',
                        height: '40px',
                        }}>
                        </Input>
                    :
                        <Input id="countNo" onChange={(e) => getHousesArg.count=e.target.value} placeholder="總筆數數量顯示（填數字）"  style={{
                            width: '100%',
                            height: '40px',
                        }}>
                        </Input>
            ...
            <Row>
                <Col  xs={24} sm={3} md={3} lg={4} xl={4}></Col>
                <Col  xs={24} sm={21} md={21} lg={20} xl={18}>
                    <Table columns={isShowDel? columns : filterdColumns}
